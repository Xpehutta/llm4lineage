...
**СДЕЛАНО (2026-08-07).** CI разделён на два job'а: `lint` (ruff + mypy, один прогон)
и `test` (матрица 3.10–3.13). Оба на `uv` с кэшем по `uv.lock`; `UV_FROZEN=1`, чтобы
устаревший лок падал явно.

- **coverage:** `--cov=Classes --cov-fail-under=80`, отчёт `coverage.xml` в артефактах.
  Текущее покрытие — **83%** без каких-либо исключений из подсчёта.
- **mypy:** `strict`, областью — `Classes/pipeline` (`follow_imports = "silent"`, так как
  остальные `Classes/*` ещё не аннотированы). Ошибок нет.
- **golden-drift:** `update_golden.py --check` + тесты в `tests/golden/test_golden.py`.
  Прежняя проверка по F1 ≥ 0.9 допускала расхождение и пропустила 8 накопившихся рёбер —
  теперь сверка точная.
- **ruff:** `["E","F","I","W","UP","B","S"]`, замечаний нет.

> **Найдено при внедрении golden-drift:** граф не был воспроизводим между запусками.
> sqlglot собирает источники колонки через `set(...)` выражений, чей хеш солится
> `PYTHONHASHSEED`, поэтому порядок веток UNION — а с ним `branch_index` и id узлов
> `literal_*` — менялся от прогона к прогону. Ветки теперь упорядочены по позиции
> в AST (`ColumnLineageExtractor._ordered_leaves`); регрессия закрыта тестом
> `test_generation_survives_hash_randomisation`, который сравнивает прогоны
> в подпроцессах с разными `PYTHONHASHSEED`.

> **Удалено:** `Classes/refine_classes.py` (271 строка) — модуль не импортировался
> нигде в проекте и имел 0% покрытия. Логика refinement живёт в `sql2graph_classes.py`.
